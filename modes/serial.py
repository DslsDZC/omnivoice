...
class EnhancedSerialMode(BaseMode):
    ...
    async def execute(self, question: str, proposals: List[str] = None,
        ...
        try:
            self._agents = self.agent_pool.get_enabled_agents()
            max_n = getattr(self.serial_config, "max_agents", 2)
            self._agents = self._agents[:max(max_n, 1)]
            if not self._agents:
                return ModeResult(success=False, final_resolution="", error="没有可用的代理")
            
            print(f"\n[串行模式] {len(self._agents)} 个代理参与")
            if self._context_proposals:
                print(f"  接收会议提议: {len(self._context_proposals)} 个")

            # ADaPT 风格：先试直接回答，失败再拆解
            direct_answer = await self._try_direct_answer(question)
            if direct_answer is not None:
                print(f"\n[直接回答] 无需拆解")
                self._steps = [StepExecution(
                    step_id=1, description=question,
                    expected_output="执行结果", final_output=direct_answer,
                )]
                self._steps[0].status = StepStatus.COMPLETED
                self._steps[0].agents_results.append(AgentStepResult(
                    agent_id=self._agents[0].id, output=direct_answer,
                    test_passed=True, test_message="", duration=0
                ))
            else:
                # 1. 任务拆解
                print("[任务拆解]")
            ...
            for idx, step in enumerate(self._steps):
                ...
                # 创建快照
                if getattr(self.serial_config, 'enable_snapshot', False):
    ...
    async def _try_direct_answer(self, question: str) -> Optional[str]:
        """ADaPT 风格：先试直接回答，失败则返回 None 触发拆解"""
        if not self._agents:
            return None
        agent = self._agents[0]
        prompt = f"{question}\n\n请直接回答，不需要步骤分解。如果有不确定的内容，说'我不确定'。"
        try:
            resp = await agent.call_api([{"role": "user", "content": prompt}], temperature=0.3)
            if resp.success and resp.content:
                content = resp.content.strip()
                # 如果agent表示不确定，触发拆解
                unsure = ["不确定", "无法", "不能", "需要更多", "需要分解", "比较复杂"]
                if any(kw in content for kw in unsure):
                    return None
                return content
        except Exception:
            pass
    ...
    async def _decompose_task(self, task: str) -> List[Dict]:
        """拆解任务"""
        agents = getattr(self, "_agents", self.agent_pool.get_enabled_agents())
        if not agents:
            return []
    ...
    async def _call_with_retry(self, agent: Agent, messages: list,
                                tools=None, temperature=0.3, max_retries=2) -> any:
        """CCB 风格：API 调用带退避重试"""
        from exception_handler import ExceptionHandler
        handler = ExceptionHandler(self.whiteboard) if hasattr(self, 'whiteboard') else None

        for attempt in range(max_retries + 1):
            response = await agent.call_api(messages, tools=tools, temperature=temperature)
            if response.success:
                if handler:
                    handler.retry_success(agent.id)
                return response

            if handler:
                should, wait = handler.should_retry(agent.id)
                if not should:
                    break
                ...
                if attempt < max_retries:
                    ...
                    print(f"  [重试] {agent.id} 第{attempt+1}次失败")
                else:
                    break
        return response
    ...
    async def _execute_step(self, step: StepExecution, step_config: Dict, question: str):
        ...
        if len(agents) <= 2:
            ...
            step.agents_results.append(AgentStepResult(
                agent_id=proposer.id, output=current_proposal,
                test_passed=True, test_message="", duration=0
            ))
        ...
        for vote_round in range(1, max_vote_rounds + 1):
            ...
            async def vote(agent):
                ...
                if resp.success and resp.content:
                    ...
                    if not speech_match:
                        speech_match = re_module.search(r'[\[【]用户摘要[\]】]?\s*(.+?)(?:\n|$)', resp.content)
                    ...
                    if speech_match:
                        ...
                        um = re_module.search(r'[\[【]用户摘要[\]】]?\s*(.+?)(?:\n|$)', resp.content)
                        if um:
                            speech = um.group(1).strip()
                    reason = reason_match.group(1).strip() if reason_match else ""
                    # 清理残留的 】
                    speech = re_module.sub(r'[】【]', '', speech).strip()
                    reason = re_module.sub(r'[】【]', '', reason).strip()
